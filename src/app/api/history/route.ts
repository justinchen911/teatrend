...
// 真实有缓存数据的账号 + 头像
const ACCOUNTS: Array<{ uid: string; name: string; avatar: string }> = [
...
interface Note {
  ...
  time_desc: string; user?: { nickname: string; images: string; userid: string }
}

interface TikHubData { code: number; data?: { notes?: Note[]; items?: Note[] } }
...
export async function GET() {
  ...
  for (const acc of ACCOUNTS) {
    ...
    if (!notes.length) continue  // 无笔记的账号不展示

    const avatar = notes[0]?.user?.images || acc.avatar
    const mapped = notes.slice(0, 5).map(buildNote)
